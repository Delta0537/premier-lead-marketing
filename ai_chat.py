...
# Use the NEW google-genai package (not deprecated google-generativeai)
try:
...
class Config:
    ...
    Key Priority (checks in order):
    - PLM_ANTHROPIC_API_KEY -> ANTHROPIC_API_KEY
    - PLM_GEMINI_API_KEY -> GEMINI_API_KEY -> hardcoded fallback
    """
    # Hardcoded fallbacks for both PLM and general use
    _PLM_ANTHROPIC_KEY: str = 'YOUR_PLM_ANTHROPIC_API_KEY_HERE'
    _ANTHROPIC_KEY: str = 'YOUR_ANTHROPIC_API_KEY_HERE'
    ANTHROPIC_API_KEY: str = os.getenv('PLM_ANTHROPIC_API_KEY') or os.getenv('ANTHROPIC_API_KEY') or _ANTHROPIC_KEY
    # Gemini API key - check PLM key first, then general, then hardcoded fallback
    GEMINI_API_KEY: str = os.getenv('PLM_GEMINI_API_KEY') or os.getenv('GEMINI_API_KEY', 'YOUR_GEMINI_API_KEY_HERE')

    CLAUDE_MODEL: str = "claude-sonnet-4-20250514"  # Or use "claude-3-5-sonnet-20241022" if date doesn't work
    GEMINI_MODEL: str = "gemini-2.5-flash"  # Gemini 2.5 Flash - latest model
...
class ClaudeClient:
    """Claude API client - SEPARATE function for Claude."""
    ...
    def chat(self, message: str, tracker: TokenTracker) -> str:
        """SEPARATE Claude chat function."""
        self.conversation.append({"role": "user", "content": message})

        # Try configured model, fallback if needed
        try:
            ...
            # Fallback to claude-3-5-sonnet if model not available
...
class GeminiClient:
    """Gemini API client - Uses new google-genai package."""
    ...
    def __init__(self, api_key: str):
        ...
        if GENAI_NEW:
            ...
            print(f"[OK] Gemini client initialized with new google-genai (model: {self.model_name})")
        else:
            ...
            try:
                ...
                print(f"[WARN] Model {Config.GEMINI_MODEL} not available, using gemini-pro: {e}")
                self.model = genai.GenerativeModel("gemini-pro")
                self.chat_session = self.model.start_chat(history=[])

    def chat(self, message: str, tracker: TokenTracker) -> str:
        """SEPARATE Gemini chat function."""
        if GENAI_NEW:
            # New google-genai package
...
class UnifiedChat:
    ...
    def __init__(self):
        ...
        # SEPARATE initialization - Claude
        if self.config.ANTHROPIC_API_KEY:
            try:
                self.claude = ClaudeClient(self.config.ANTHROPIC_API_KEY)
                print("[OK] Claude initialized (SEPARATE function)")
            except Exception as e:
                print(f"[WARN] Claude init failed: {e}")
                print(f"[INFO] Check ANTHROPIC_API_KEY - may need new key")
                self.claude = None
        else:
            print("[WARN] ANTHROPIC_API_KEY not set - Claude disabled")
            print("[INFO] Set with: $env:ANTHROPIC_API_KEY='your-key'")

        # SEPARATE initialization - Gemini
        if self.config.GEMINI_API_KEY:
            try:
                self.gemini = GeminiClient(self.config.GEMINI_API_KEY)
                print("[OK] Gemini initialized (SEPARATE function)")
            except Exception as e:
                print(f"[WARN] Gemini init failed: {e}")
                print(f"[INFO] Check GEMINI_API_KEY - may need new key")
                self.gemini = None
        else:
            print("[WARN] GEMINI_API_KEY not set - Gemini disabled")
            print("[INFO] Set with: $env:GEMINI_API_KEY='your-key'")
...
def main():
    ...
    # Check for API keys - Gemini has a default key, so only warn if neither is set
    config = Config()
